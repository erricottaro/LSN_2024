...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double sum_neighbours = _particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin();
    double prob_up = 1./(1. + exp(-_beta*2.0*(_J*sum_neighbours + _H))); //probability of spin being set to 1
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8.0*M_PI*_rho/3.*(1./(3.0*pow(_r_cut, 9))-1./pow(_r_cut, 3));
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr) {
          int index_bin;
          //Floor of ration between distance and bin size, gives index of bin
          if(dr < _halfside.min()) {
            index_bin = int(dr/_bin_size);
            _measurement(_index_gofr+index_bin)+=2; //GOFR
          }
        }
        if(dr < _r_cut){
  ...
  if (_measure_pressure and _measure_temp){
    pressure_temp = _ptail + _rho*_measurement(_index_temp) + 1.0/(3*_volume)*48.*pressure_temp;
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv and _measure_tenergy) _measurement(_index_cv) = (_npart*_measurement(_index_tenergy))*(_npart*_measurement(_index_tenergy));
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi and _measure_magnet) _measurement(_index_chi) = _beta*_measurement(_index_magnet)*_measurement(_index_magnet);
...
void System :: averages(int blk){
  ...
  double cv_dummy, cv2_dummy;
  if(_measure_cv) {
  cv_dummy=_global_av(_index_cv);
  cv2_dummy=_global_av2(_index_cv);
  }
  ...
  if(_measure_cv) {
  _global_av(_index_cv)=cv_dummy;
  _global_av2(_index_cv)=cv2_dummy;
  }
  ...
  if (_measure_penergy){
    ...
    coutf << scientific << setprecision(18)
          << setw(12) << blk 
          << " " << setw(20) << average
  ...
  if (_measure_gofr){
    if(blk == _nblocks){
      ...
      {
        //infinitesimal volume (r is midpoint of bin)
        double delta_v=4.*M_PI/3.*pow(_bin_size, 3)*(pow(i+1.5, 3)-pow(i+0.5, 3));
        double normal = _rho*_npart*delta_v;
        average  = _average(_index_gofr+i)/normal;
        sum_average = _global_av(_index_gofr+i)/normal;
        sum_ave2 = _global_av2(_index_gofr+i)/(normal*normal);
  ...
  if (_measure_cv){
    ...
    //average works fine
    average  = _average(_index_cv)-_npart*_npart*(_average(_index_tenergy)*_average(_index_tenergy));
    average *= (_beta*_beta);
    //here compute right specific heat
