...
double Random::Sample(double* events, double* probab, int dim) {
    ...
    int event_index;    // Index of the selected event
    double inf_sum = 0; // Lower bound of the current interval sum
    double sup_sum = probab[0]; // Upper bound of the current interval sum
    double r = Rannyu(); // Generate a random number between 0 and 1

    // Loop through the events array to find the event based on the probability distribution
    for (int i = 0; i < dim; i++) {
        // Check if the random number falls within the current interval
        if (inf_sum < r && sup_sum > r) {
            event_index = i; // Set the index of the selected event
            break; // Exit the loop once an event is found
        }
        // Update the interval sums for the next iteration
        inf_sum += probab[i];
        sup_sum += probab[i + 1];
    }

    return events[event_index]; // Return the selected event
}
