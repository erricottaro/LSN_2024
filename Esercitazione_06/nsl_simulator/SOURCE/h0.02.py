...
#Copia la magnetizzazione di output in apposite cartelle
def copia_output(T, method):
    dir_name = f"../../{method}"+f"{format(T, '.3f')}"
    #print(dir_name)
    comando_dir = "mkdir -p "+dir_name
    comando_copia = "cp ../OUTPUT/magnetization.dat "+dir_name
    #print(comando_dir)
    #print(comando_copia)
...
#Modifica il tipo della simulazione (campo esterno non nullo)
    ...
    with open(filename, 'w') as file:
        for linea in linee:
            if linea.startswith("SIMULATION_TYPE"):
                ...
                nuova_linea = f"SIMULATION_TYPE        {value} 1.0 0.02\n"
                file.write(nuova_linea)
            else:
                # Scrivi le altre righe del file senza modificarle
                file.write(linea)
