...
public:
  ...
  // Method to generate a random number with a exponential distribution [0, inf)
  double Exponential(double lambda);
  // Method to generate a random number with a Lorentzian distribution
  double Lorentz(double mean, double gamma);
  // Method to generate a random number with a sine distribution between 0 and pi
  double Sine();
  // Method to sample a discrete distribution of E_k events with probabilities p_k (still to be optimized with binary search)
  double Sample(double*, double*, int);
};
